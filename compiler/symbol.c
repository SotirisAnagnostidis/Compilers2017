...
static unsigned int   hashTableSize;   /* Μέγεθος πίνακα κατακερματισμού */
 SymbolEntry ** myhashTable;       /* Πίνακας κατακερματισμού        */
...
const my_Type typeVoid    = &(typeConst[0]);
const my_Type typeInteger = &(typeConst[1]);
const my_Type typeBoolean = &(typeConst[2]);
const my_Type typeChar    = &(typeConst[3]);
const my_Type typeReal    = &(typeConst[4]);
...
void initSymbolTable (unsigned int size)
{
    ...
    hashTableSize = size;
    myhashTable = (SymbolEntry **) new_h(size * sizeof(SymbolEntry *));
...
void destroySymbolTable ()
{
    ...
    for (i = 0; i < hashTableSize; i++)
        if (myhashTable[i] != NULL)
            destroyEntry(myhashTable[i]);

   delete_h(myhashTable);
}

void openScope ()
{
    Scope * newScope = (Scope *) new_h(sizeof(Scope));
...
void closeScope ()
{
    ...
    while (e != NULL) {
        ...
        myhashTable[e->hashValue] = e->nextHash;
        destroyEntry(e);
        e = next;
    }
    
    t->entries = NULL;
    currentScope = t->parent;
    t->parent = NULL;
    delete_h(t);
}
...
{
    e->nextHash             = myhashTable[e->hashValue];
    myhashTable[ e->hashValue ] = e;
...
static SymbolEntry * newEntry (const char * name)
{
    ...
    e = (SymbolEntry *) new_h(sizeof(SymbolEntry));
    e->id = (char *) new_h(strlen(name) + 1);
...
SymbolEntry * newVariable (const char * name, my_Type type, Dimensions dim)
{
    SymbolEntry * e = newEntry(name);
    if (e != NULL) {
        e->entryType = ENTRY_VARIABLE;
        e->u.eVariable.type = type;
        e->u.eVariable.dim = dim;
        type->refCount++;
    ...
    switch (type->kind) {
        ...
        case TYPE_ARRAY:
            if (equalType(type->refType, typeChar)) {
                ...
                value.vString = (const char *) new_h(strlen(str) + 1);
...
SymbolEntry * newFunction (const char * name)
{
    ...
    if (e == NULL) {
	    ...
	    e->u.eFunction.hasdeclared = 0 ;
        }
        return e;
   }
   else if (e->entryType == ENTRY_FUNCTION) {
	e->u.eFunction.hasdeclared = 1 ;
        e->u.eFunction.isForward = false;
                            ...
                            PassMode mode, SymbolEntry * f)
{
    ...
    switch (f->u.eFunction.pardef) {
		...
		e->u.eParameter.dim = dim;
            ...
            e->u.eParameter.position = position;
            return e;            
        case PARDEF_CHECK:
    ...
    switch (e->entryType) {
        ...
        case ENTRY_CONSTANT:
            if (e->u.eConstant.type->kind == TYPE_ARRAY)
                delete_h((char *) (e->u.eConstant.value.vString));
...
SymbolEntry * lookupEntry (const char * name, LookupType type, bool err)
{
    ...
    SymbolEntry * e         = myhashTable[hashValue];
...
{
    my_Type n = (my_Type) new_h(sizeof(struct Type_tag));
